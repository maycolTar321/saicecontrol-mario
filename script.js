document.addEventListener('DOMContentLoaded', () => {
    ...
    function drawUtilityChart(xOpt, utMax, m, c, cv, cf) {
        ...
        utilityChart = new Chart(canvas.getContext('2d'), {
            ...
            options: {
                ...
                scales: {
                    x: { type: 'linear', display: true, grid: { display: false }, ticks: { color: '#fff' }, border: { color: '#fff', width: 2 }, min: -xOpt * 0.1, max: xOpt * 2.1 },
                    y: { type: 'linear', display: true, grid: { display: false }, ticks: { color: '#fff' }, border: { color: '#fff', width: 2 }, min: -utMax * 0.1, max: utMax * 1.2 }
                }
            }
        });
    }
    ...
    function drawImperfectaChart(canvasId, m, c, cv, xMono, xComp) {
        ...
        imperfectaChart = new Chart(canvas.getContext('2d'), {
            ...
            data: {
                datasets: [
                    { label: 'P', data: demData, showLine: true, borderColor: '#3b82f6', borderWidth: 3, pointRadius: 0 },
                    { label: 'IM', data: imData, showLine: true, borderColor: '#10b981', borderWidth: 3, pointRadius: 0 },
                    { label: 'CM', data: cmData, showLine: true, borderColor: '#ef4444', borderWidth: 3, pointRadius: 0 }
                ]
            },
            options: {
                ...
                scales: { 
                    x: { type: 'linear', display: true, grid: { display: false }, ticks: { color: '#fff' }, border: { color: '#fff', width: 2 }, min: -xComp * 0.1, max: xComp * 1.5 }, 
                    y: { type: 'linear', display: true, grid: { display: false }, ticks: { color: '#fff' }, border: { color: '#fff', width: 2 }, min: -c * 0.1, max: c * 1.2 } 
                }
            }
        });
    }
